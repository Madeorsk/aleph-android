...
- **Colors**: prefer an existing upstream theme attribute when one fits the role (`values/attrs.xml` declares `colorFavorite`, `colorBoost`, `colorBookmark` and the M3 roles, all bound in every theme including the contrast variants). When none fits, Aleph colors go in `values/aleph_colors.xml`, never in `values/colors.xml`. Do not add a `values-night` variant: the app forces its theme from `GlobalUserPreferences.theme`, so the night qualifier disagrees with the active theme whenever the preference and the system disagree. Use a theme attribute instead.
